...
import { getInnovations, deleteInnovationById } from 'apiSdk/innovations';
import { InnovationInterface } from 'interfaces/innovation';

type ColumnType = ColumnDef<InnovationInterface, unknown>;

interface InnovationListPageProps {
...
export function InnovationListPage(props: InnovationListPageProps) {
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<InnovationInterface>>(
    () => `/innovations?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: InnovationInterface) => {
    if (hasAccess('innovation', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/innovations/view/${row.id}`);
    }
  };
      ...
      : null,
    {
      ...
      cell: ({ row: { original: record } }: any) => (
        <Flex justifyContent="flex-end">
          <NextLink href={`/innovations/view/${record.id}`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              mr={2}
              padding="0rem 8px"
          ...
          {hasAccess('innovation', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
            <NextLink href={`/innovations/edit/${record.id}`} passHref legacyBehavior>
              <Button
                onClick={(e) => e.stopPropagation()}
                mr={2}
                padding="0rem 0.5rem"
          ...
          {hasAccess('innovation', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
            <IconButton
              onClick={(e) => {
                e.stopPropagation();
    ...
    <Box p={4} rounded="md" shadow="none">
      <Flex justifyContent="space-between" mb={4}>
        <Flex alignItems="center" gap={1}>
          ...
          <AccessInfo entity="innovation" />
        </Flex>

        {hasAccess('innovation', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
          <NextLink href={`/innovations/create`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              height={'2rem'}
